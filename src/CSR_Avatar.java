...
public class CSR_Avatar extends A_GameObject {
    ...
    private final double accConst = 1.005;
    private final double stoppingThreshold = 0.01;
    private final double accelerationThreshold = 1.0;
    private final double startSpeed = 0.01;
    ...
    void move(double diffSec) {
        if(!onTheGround) {
            //First speed calculations with multiplication/division of a constant for acceleration.
            //if(isAscending)
            //  if(speed >= 0) speed *= c / diffSec;
            //    else if(speed < threshold) speed /= diffSec * c; else speed = 0;
            // //descending:
            //else if(speed <= 0) speed *= c / diffSec;
            //      else if(speed > threshold) speed /= diffsec * c; else speed = 0;
            if (isAscending) {                                                            //If we hold space
                if (Math.abs(speed) < accelerationThreshold) {                                     //and if we aren't already at full speed ...
